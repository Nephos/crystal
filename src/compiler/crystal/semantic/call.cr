...
class Crystal::Call
    ...
    args.each_with_index do |arg, i|
      ...
      when Splat
        ...
        when TupleInstanceType
          tuple_types = arg_type.tuple_types

          # If the last element in the tuple is a named argument, we double splat it
          last_tuple_type = tuple_types.last?
          if last_tuple_type.is_a?(NamedTupleInstanceType)
            arg_types.concat tuple_types[0...-1]
            named_args_types = add_named_args_types(named_args_types, last_tuple_type)
          else
            arg_types.concat tuple_types
          end
        when UnionType
      ...
      when DoubleSplat
        ...
        when NamedTupleInstanceType
          named_args_types = add_named_args_types(named_args_types, arg_type)
        when UnionType
  ...
  private def add_named_args_types(named_args_types, named_tuple_type)
    named_tuple_type.names_and_types.each do |name_and_type|
    ...
    args.each_with_index do |arg, i|
      ...
      when Splat
        ...
        arg_type.tuple_types.each_with_index do |tuple_type, index|
          ...
          # If the last element in the tuple is a named tuple, we double splat it
          if tuple_type.is_a?(NamedTupleInstanceType) && index == arg_type.tuple_types.size - 1
            tuple_type.names_and_types.each do |name_and_type|
              sym = SymbolLiteral.new(name_and_type[0])
              sym.type = mod.symbol
              mod.symbols.add sym.value

              cloned_tuple_indexer = tuple_indexer.clone
              cloned_tuple_indexer.obj.not_nil!.type = arg.exp.type
              cloned_tuple_indexer.args.first.type = num.type
              parent_visitor.prepare_call(cloned_tuple_indexer)
              cloned_tuple_indexer.recalculate

              double_tuple_indexer = Call.new(cloned_tuple_indexer, "[]", sym)
              parent_visitor.prepare_call(double_tuple_indexer)
              double_tuple_indexer.recalculate

              new_args << double_tuple_indexer
            end
          else
